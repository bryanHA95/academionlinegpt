        ...
        <div class="order-1 lg:order-2">
            <section class="card mb-4">
                <div class="card-body">
                    ...
                    @can('enrolled', $course)
                        <a class="font-bold py-2 px-4 rounded bg-red-500 hover:bg-red-700 text-white block text-center w-full mt-4" href="{{route('courses.status', $course)}}">Continuar el Curso</a>
                    @else
                        @if ($course->price->value == 0)
                            <p class="text-2xl font-bold text-gray-500 mt-3 mb-2">GRATIS</p>
                            <form action="{{route('courses.enrolled', $course)}}" method="post">
                                @csrf
                                <button class="font-bold py-2 px-4 rounded bg-red-500 hover:bg-red-700 text-white block text-center w-full" type="submit">Matricularse</button>
                            </form>
                        @else
